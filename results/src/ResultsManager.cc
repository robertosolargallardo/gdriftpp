...
ResultsManager::ResultsManager(const string &_uri_name){
	this->_mongo=make_shared<Util::MongoDB>(_uri_name);
}
...
boost::property_tree::ptree ResultsManager::pca(const uint32_t &_id){
	list<boost::property_tree::ptree> fdata=this->_mongo->read("gdrift","data",_id);
	list<boost::property_tree::ptree> fresults=this->_mongo->read("gdrift","results",_id);
...
boost::property_tree::ptree ResultsManager::estimations(const uint32_t &_id){
	list<boost::property_tree::ptree> fsettings=this->_mongo->read("gdrift","settings",_id);
	...
	list<boost::property_tree::ptree> results=this->_mongo->read("gdrift","results",(*fsettings.begin()).get<uint32_t>("id"));
...
boost::property_tree::ptree ResultsManager::generate(const uint32_t &_id){
...
boost::property_tree::ptree ResultsManager::get_results(void){
	list<boost::property_tree::ptree> fsettings=this->_mongo->read_all("gdrift","settings");

	boost::property_tree::ptree fresults,ftuples;
	for(auto fsetting : fsettings){
		boost::property_tree::ptree ftuple;
		ftuple.put("id",fsetting.get<string>("id"));
		ftuples.push_back(make_pair("",ftuple));
	}
