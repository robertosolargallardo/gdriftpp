...
void ConfigGenerator::sendBatch( ){
   ...
   for(auto& it : configurations) {
      ...
      write_json(config_ss, it.second);
      sendConfiguration( &config_ss );
   }
   configurations.clear( );
}

bool ConfigGenerator::sendConfiguration( std::stringstream *ss ){
   bool response=true;
   ...
   curl_easy_setopt(curl,CURLOPT_URL,"http://localhost:1985/controller");
